...
def local_train(
    ...
    for ep in range(start_step.ep, end_step.ep + 1):
        ...
        for it, (examples, labels) in enumerate(train_loader):
            ...
            # TODO: make this part local
            step_optimizer.zero_grad()
            model.train()
            loss = model.compute_train_loss(examples, labels)
            if training_hparams.apex_fp16:
                with apex.amp.scale_loss(loss, optimizer) as scaled_loss:
                    scaled_loss.backward()
            else:
...
def standard_train(
  ...
  evaluate_every_epoch: bool = True
    ...
    local_train(training_hparams, model, train_loader, output_location, callbacks, start_step=start_step)
